...
describe('TikTok quirks', () => {
  it('reports failure as HTTP 200 with the error in the body', async () => {
    const res = await fetch(`${process.env.TIKTOK_BASE_URL}/v2/comment/reply/`, {
      method: 'POST',
      headers: {
        authorization: 'Bearer tt_token_primary',
        'content-type': 'application/json',
      },
      body: JSON.stringify({ video_id: 'tt_post_a', comment_id: 'tt_missing', text: 'hi' }),
    });
  ...
  it('client throws on the 200-hidden error instead of reporting success', async () => {
    await expect(
      clientFor('tiktok').postReply(
        { platformPostId: 'tt_post_a', parentPlatformCommentId: 'tt_missing', body: 'hi' },
